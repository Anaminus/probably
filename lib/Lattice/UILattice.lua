...
local function updateConstraints(parent, min, attributes)
	local constraints
	if attributes then
		constraints = attributes.Constraints
	else
		constraints = parent:GetAttribute(ATTR_CONSTRAINTS)
	end
...
--@ord: 1
--@def: function UILattice.update(parent: Instance, attributes: Attributes): Instance
...
--
-- If *attributes* is specified, then it will be used instead.
--
...
function UILattice.update(parent, attributes)
	...
	local attrPadding
	if attributes then
		attrColumns = attributes.Columns
		attrRows = attributes.Rows
		attrMargin = attributes.Margin
		attrPadding = attributes.Padding
	else
		attrColumns = parent:GetAttribute(ATTR_COLUMNS)
		attrRows = parent:GetAttribute(ATTR_ROWS)
		attrMargin = parent:GetAttribute(ATTR_MARGIN)
		attrPadding = parent:GetAttribute(ATTR_PADDING)
	end

	local err, coln, colu = parseSpans(attrColumns)
	if err ~= nil then
		error(string.format("parse columns: %s", err), 2)
	end
	local err, rown, rowu = parseSpans(attrRows)
	if err ~= nil then
		error(string.format("parse rows: %s", err), 2)
	end
	local err, margin = parseNumber(attrMargin)
	if err ~= nil then
		error(string.format("parse margin: %s", err), 2)
	end
	local err, padding = parseNumber(attrPadding)
	if err ~= nil then
	...
	updateConstraints(parent, Vector2.new(colmin, rowmin), attributes)
